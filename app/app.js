...
function initListeners() {
    $("nav a").click(function (e) { //checks the nav a tags for clicks
        let pageId = e.currentTarget.id; //grabs the #id from the clicked link
        loadPageContent(pageId) //pass pageId into the loadPageContent function
    });
}

function loadPageContent(pageName) {
    let contentID = pageName + "Content"; //appends Content to the end of the gathered ID
    $('#app').html(eval(contentID)); //replaces all content in html, eval translates the new string to a variable that is executed 
    initListeners(); 
}
...
$(document).ready(function () { //when the document is loaded 
    loadPageContent("home"); //display the home page variable
    initApp();
}) 